...
export default function ProductDetailsPage() {
  ...
  const fetchProductDetails = async () => {
    ...
    try {
      ...
      if (data.success && (data.product || data.project)) {
        const item = data.product || data.project;
        setProduct(item);
        setCategoryRank(data.categoryRank || 1);
        setMinOutbidPrice(data.minOutbidPrice || item.totalBid + 1);
      }
    } catch (err) {
  ...
  const handleCopyLink = () => {
    ...
    const link = `${origin}/product/${product?.id || id}`;
      ...
      <div className="min-h-screen bg-[#fafafa] dark:bg-[#09090b] flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs text-zinc-500 font-mono">Loading product details...</p>
          </div>
        </div>
        <Footer />
      </div>
        ...
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="text-center max-w-md p-8 rounded-3xl bg-white dark:bg-[#121217] border border-zinc-200 dark:border-[#272732] shadow-sm">
            ...
            <p className="text-xs text-zinc-500 mt-2 mb-6">
              The product you are looking for has not been claimed yet or the URL is invalid.
            </p>
            <Link
              href="/"
  ...
  const displayTitle = formatProjectTitle(product);
  const cleanDomain = getCleanDomain(product.url || product.normalizedUrl);
  const joinedTime = formatJoinedTime(product.createdAt);
  const exactDate = formatExactDate(product.createdAt);
      ...
      <main className="w-full max-w-5xl mx-auto px-4 sm:px-6 pt-6 pb-24">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-[#121217] border border-zinc-200/80 dark:border-[#272732] shadow-xs relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
            {/* Left: Avatar with Tilted Upward Crown Above + Title + Bio + Metadata */}
            <div className="flex items-start gap-4 sm:gap-5 flex-1 min-w-0">
              {/* Product Logo with Tilted Crown Sitting Above */}
              <div className="relative shrink-0 pt-2">
                ...
                {/* Tilted Upward Crown Sitting Majestically ABOVE the Logo */}
                {product.rank <= 3 && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 -rotate-12 pointer-events-none drop-shadow-[0_4px_8px_rgba(0,0,0,0.25)]">
                    {getCrown(product.rank)}
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-2 text-sm sm:text-base text-zinc-700 dark:text-zinc-300 leading-relaxed font-normal">
                  {product.description ||
                    `Discover ${displayTitle} — ranked #${product.rank} on the Outbid King leaderboard.`}
                </p>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400 font-medium">
                  ...
                  <span className="px-3 py-1 rounded-xl bg-zinc-100 dark:bg-[#181822] text-[#ea6c52] font-bold flex items-center gap-1.5">
                    <CategoryIcon slug={product.category} size="xs" />
                    <span>{catInfo ? catInfo.name : product.category}</span>
                  </span>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-5 rounded-3xl bg-white dark:bg-[#121217] border border-zinc-200/80 dark:border-[#272732] shadow-xs">
            ...
            <div className="mt-2 text-3xl font-extrabold font-mono text-[#ea6c52]">
              ${product.totalBid.toLocaleString()}
            </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white tracking-tight">
                  Outbid {displayTitle} & Take Rank #{product.rank}
                </h3>
              </div>
              <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 mt-1 max-w-xl leading-relaxed">
                Current investment is <span className="font-mono font-bold text-zinc-900 dark:text-white">${product.totalBid}</span>.
